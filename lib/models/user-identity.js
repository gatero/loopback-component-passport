...
module.exports = function(UserIdentity) {
  ...
  UserIdentity.login = function(provider, authScheme, profile, credentials, options, cb) {
    ...
    if (profile.emails === undefined) {
      return cb('We need the email address of your Facebook profile');
    }
    ...
    .then(function(invitedUser) {
      if (!invitedUser) {
        process.nextTick(function() {
          return cb(email + ' is not a recognized user');
        });
        return;
      }

      // If the status of the invitedUser === 'pending',
      // it means it's a Brand New User™
      // We need to change it's status to 'active'
      ...
      userIdentityModel.findOne({where: {
        ...
        if (identity) {
          ...
          return identity.updateAttributes({profile: profile,
            ...
            return identity.user(function(err, user) {
              if (user && user.status === 'inactive') return cb(email + ' is not active');
        ...
        userModel.findOrCreate({where: query}, userObj, function(err, user) {
          if (user && user.status === 'inactive') return cb(userObj.email + ' is not active');
          if (err) {
            return cb(err);
          }
          var date = new Date();
